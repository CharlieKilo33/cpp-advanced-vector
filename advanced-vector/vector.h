...
class Vector {
 public:
  ...
  void InitializeOrDestroy(const Vector& rhs) {
    std::copy(rhs.data_.GetAddress(),
              std::min(rhs.data_.GetAddress() + size_,
  ...
  void PushBack(const T& value) { EmplaceBack(std::forward<const T&>(value)); }

  void PushBack(T&& value) { EmplaceBack(std::forward<T&&>(value)); }
